...
const HOSPITAL = {
  funcionarios: 320, salarioMedio: 3800, turnover: 0.28,
  absenteismo: 0.074, diasUteis: 22, meses: 12, fatorEncargos: 1.70,
};

const calcMetrics = () => {
  const { funcionarios, salarioMedio, turnover, absenteismo, diasUteis, meses, fatorEncargos } = HOSPITAL;
  const folhaAnual = funcionarios * salarioMedio * 13.3 * fatorEncargos;
  const desligamentos = Math.round(funcionarios * turnover);
  const custoTurnover = desligamentos * 3 * salarioMedio * fatorEncargos;
  const diasPerdidos = Math.round(funcionarios * diasUteis * meses * absenteismo);
  const salarioDia = salarioMedio / diasUteis;
  const custoAbsenteismo = Math.round(diasPerdidos * salarioDia + diasPerdidos * salarioDia * 0.30);
  const multasNR1 = funcionarios * 6708;
  const passivoTrabalhista = Math.round(folhaAnual * 0.02);
  ...
  const payback = layers.map(l => {
    const economiaAnual = Math.round(desligamentos * l.reducaoTurnover * 3 * salarioMedio * fatorEncargos + diasPerdidos * l.reducaoAbsenteismo * salarioDia * 12);
  ...
  return { folhaAnual: Math.round(folhaAnual), desligamentos, custoTurnover, diasPerdidos, custoAbsenteismo, multasNR1, passivoTrabalhista, totalExposicao, payback, setores, trend };
};

// Apenas CFO acessa ROI
router.get('/metrics', auth, requireRole('cfo'), (req, res) => {
