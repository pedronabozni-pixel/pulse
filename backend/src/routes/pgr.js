...
// RH tem acesso total; Jurídico tem somente leitura
const canRead  = requireRole('rh', { readOnly: ['juridico'] });
const canWrite = requireRole('rh');

router.get('/risks', auth, canRead, (req, res) => {
  const risks = db.findAll('risks', { status: 'Ativo' })
    .sort((a, b) => (b.probabilidade * b.severidade) - (a.probabilidade * a.severidade));
  res.json(risks);
});

router.post('/risks', auth, canWrite, (req, res) => {
...
router.put('/risks/:id', auth, canWrite, (req, res) => {
...
router.delete('/risks/:id', auth, canWrite, (req, res) => {
  db.update('risks', { id: +req.params.id }, { status: 'Inativo' });
  res.json({ success: true });
});

router.get('/actions', auth, canRead, (req, res) => {
...
router.post('/actions', auth, canWrite, (req, res) => {
...
router.put('/actions/:id', auth, canWrite, (req, res) => {
...
router.get('/history', auth, canRead, (req, res) => {
  const history = db.findAll('pgrHistory').sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
  const users = db.findAll('users');
  res.json(history.map(h => ({ ...h, author: users.find(u => u.id === h.created_by)?.name || 'Sistema' })));
});

router.post('/history', auth, canWrite, (req, res) => {
...
router.get('/export', auth, canRead, (req, res) => {
  const risks = db.findAll('risks', { status: 'Ativo' }).sort((a, b) => (b.probabilidade * b.severidade) - (a.probabilidade * a.severidade));
  const allActions = db.findAll('actions');
  const allRisks = db.findAll('risks');
  const actions = allActions.map(a => ({ ...a, perigo: allRisks.find(r => r.id === a.risk_id)?.perigo || null, area: allRisks.find(r => r.id === a.risk_id)?.area || null }));
